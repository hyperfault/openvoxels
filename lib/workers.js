...
async function callWorker(modelId, prompt) {
    ...
    if (worker.provider === "gemini") return callGemini(modelId, prompt);
    if (worker.provider === "huggingface") return callHuggingFace(modelId, prompt);
    throw new Error(`Unknown provider for ${modelId}`);
}

export async function runWorkers(prompt, workerCount, customModels = null) {
    const modelIds = (customModels && customModels.length >= workerCount)
